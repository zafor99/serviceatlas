 ...
 */
public  class SAPCallingiDocTestToolDialogModel extends SAPModelBase
{
private SAPApplicationModel appModel = new SAPApplicationModel();
	
	public SAPCallingiDocTestToolDialogModel(){
	...
	public SAPTopLevelTestObject dialog(){
		...
		TestObject[] tos = null;
		try {
			tos = SAPSession.getSession().find(atDescendant("Id","/wnd[1]","Text","Calling IDoc test tool in productive client"));
			if(tos.length>0){
