...
import resources.scripts.outboudsapactions.withoutingestion.giftcardnCC.TC01_CancelHelper;
...
public class TC01_Cancel extends TC01_CancelHelper
{
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(orderNumber.length()>2){
			writeOutboundGCOrdersToExcel("TC01_Cancel", timeStamp, orderNumber, ean,customerID);
			...
			String iDocForZR88 = sap().iDocReportingToolDialog().getIDocForZR88SubscCancelled();
			String statusForZR88 = sap().iDocReportingToolDialog().getStatusForZR88SubscCancelled();
//			sap().iDocReportingToolDialog().verifyIDocReportingToolTable("Z1116_Status");
			System.out.println("iDoc Number for ZR88 : "+ iDocForZR88 + " and status : "+statusForZR88);

			if(statusForZR88.contains("30")){
				sap().iDocReportingToolDialog().topLevelToolbar().enterCommand("/nbd87");
				sap().selectIDocDialog().searchIDoc(iDocForZR88);
				sap().statusMonitorDialog().expandiDocOutboundProcessing();
				sap().statusMonitorDialog().selectORDRSPndProcess();
				sap().informationDialog().clickOkButton();
				sap().iDocProcessingDialog().verifyProcessedIDOCGrid("ZR88_processed");
