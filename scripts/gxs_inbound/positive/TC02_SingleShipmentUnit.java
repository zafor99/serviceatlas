...
import resources.scripts.gxs_inbound.positive.TC02_SingleShipmentUnitHelper;
 ...
 * @author zsadeque
 */
public class TC02_SingleShipmentUnit extends TC02_SingleShipmentUnitHelper
{
	/**
	...
	{
		atlas().advanceShipNotice().loadXmlFile("gxs_inbound\\Positive\\TC02_SingleShipmentUnit.xml");
		atlas().advanceShipNotice().setRandomReferenceID();
		atlas().advanceShipNotice().setHeader();
		atlas().advanceShipNotice().postInternal();
		atlas().advanceShipNotice().verifyResponse("TC02_SingleShipmentUnit", "201");
