...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC14_eTextbookPurchaseHelper;
...
public class TC14_eTextbookPurchase extends TC14_eTextbookPurchaseHelper
{
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		ean = "2940043770806";//"9780826260383";
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC14_eTextbookPurchase", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC14_eTextbookPurchase");
			customerID = getCustomerIDFromBNOrdersExcel("TC14_eTextbookPurchase");
