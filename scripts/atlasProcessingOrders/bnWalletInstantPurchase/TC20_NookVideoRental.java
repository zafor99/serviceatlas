...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC20_NookVideoRentalHelper;
 ...
 * @author zsadeque
 */
public class TC20_NookVideoRental extends TC20_NookVideoRentalHelper
{
	/**
	 ...
	 * @throws IOException 
	 */
	public void testMain(Object[] args) throws ClassNotFoundException, IOException 
	{
		String orderNumber,customerID,timeStamp,ean = null;
		ean = "2949000360852";
		
		//ean = "2949000628587";2949000000284
		if(AtlasScriptExecution.getScriptRun())
		{
			instantPurchase().submitVideoRentalIPOrder(ean);
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC20_NookVideoRental", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC20_NookVideoRental");
			customerID = getCustomerIDFromBNOrdersExcel("TC20_NookVideoRental");
			...
			if(orderNumber.length()>2){
				...
				digitalLocker(customerID).verifyVideoLockerItem("DigitalLocker");
