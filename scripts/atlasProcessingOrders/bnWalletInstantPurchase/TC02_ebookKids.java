...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC02_ebookKidsHelper;
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC02_ebookKids", timeStamp, orderNumber, ean,customerID);
			}	
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC02_ebookKids");
			customerID = getCustomerIDFromBNOrdersExcel("TC02_ebookKids");
