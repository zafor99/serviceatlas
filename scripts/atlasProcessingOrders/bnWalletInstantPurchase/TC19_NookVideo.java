...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC19_NookVideoHelper;
	...
	public void testMain(Object[] args) throws ClassNotFoundException, IOException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC19_NookVideo", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC19_NookVideo");
			customerID = getCustomerIDFromBNOrdersExcel("TC19_NookVideo");
