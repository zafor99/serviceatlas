...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC21_eMagazineSubscriptionAnnualHelper;
 ...
 * @author zsadeque
 */
public class TC21_eMagazineSubscriptionAnnual extends TC21_eMagazineSubscriptionAnnualHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC21_eMagazineSubscriptionAnnual", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC21_eMagazineSubscriptionAnnual");
			customerID = getCustomerIDFromBNOrdersExcel("TC21_eMagazineSubscriptionAnnual");
