...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC18_NookAppHelper;
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC18_NookApp", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC18_NookApp");
			customerID = getCustomerIDFromBNOrdersExcel("TC18_NookApp");
