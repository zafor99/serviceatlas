...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC17_eBookNonAgencyHelper;
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC17_eBookNonAgency", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC17_eBookNonAgency");
			customerID = getCustomerIDFromBNOrdersExcel("TC17_eBookNonAgency");
