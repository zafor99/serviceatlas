...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC11_eNewspaperSubscriptionAgencyHelper;
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC11_eNewspaperSubscriptionAgency", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC11_eNewspaperSubscriptionAgency");
			customerID = getCustomerIDFromBNOrdersExcel("TC11_eNewspaperSubscriptionAgency");
