...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC09_SingleIssue_ENewspaperHelper;
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC09_SingleIssue_ENewspaper", timeStamp, orderNumber, ean,customerID);
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC09_SingleIssue_ENewspaper");
			customerID = getCustomerIDFromBNOrdersExcel("TC09_SingleIssue_ENewspaper");
