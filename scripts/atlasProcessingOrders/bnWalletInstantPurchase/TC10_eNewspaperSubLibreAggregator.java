...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC10_eNewspaperSubLibreAggregatorHelper;
...
public class TC10_eNewspaperSubLibreAggregator extends TC10_eNewspaperSubLibreAggregatorHelper
{
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		ean = "2940000281581";//"2940000281581";//"2940000446416";
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC10_eNewspaperSubLibreAggregator", timeStamp, orderNumber, ean,customerID);
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC10_eNewspaperSubLibreAggregator");
			customerID = getCustomerIDFromBNOrdersExcel("TC10_eNewspaperSubLibreAggregator");
