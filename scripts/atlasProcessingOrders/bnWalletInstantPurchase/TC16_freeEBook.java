...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC16_freeEBookHelper;
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC16_freeEBook", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC16_freeEBook");
			customerID = getCustomerIDFromBNOrdersExcel("TC16_freeEBook");
