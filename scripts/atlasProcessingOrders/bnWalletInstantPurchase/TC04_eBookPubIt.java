...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC04_eBookPubItHelper;
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC04_eBookPubIt", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC04_eBookPubIt");
			customerID = getCustomerIDFromBNOrdersExcel("TC04_eBookPubIt");
