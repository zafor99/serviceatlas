...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC07_eMagazineSIPHelper;
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC07_eMagazineSIP", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC07_eMagazineSIP");
			customerID = getCustomerIDFromBNOrdersExcel("TC07_eMagazineSIP");
