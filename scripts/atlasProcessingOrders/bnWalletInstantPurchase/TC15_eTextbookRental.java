...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC15_eTextbookRentalHelper;
...
public class TC15_eTextbookRental extends TC15_eTextbookRentalHelper
{
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		ean = "2940043770806";//9780495896807

		if(AtlasScriptExecution.getScriptRun())
		{
			instantPurchase().submitIPRentalOrder("VI", "4313081835209051", ean, "180", false);
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC15_eTextbookRental", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC15_eTextbookRental");
			customerID = getCustomerIDFromBNOrdersExcel("TC15_eTextbookRental");
