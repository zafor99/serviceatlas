...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC25_SparkNotesHelper;
 ...
 * @author zsadeque
 */
public class TC25_SparkNotes extends TC25_SparkNotesHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC25_SparkNotes", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC25_SparkNotes");
			customerID = getCustomerIDFromBNOrdersExcel("TC25_SparkNotes");
			...
			if(orderNumber.length()>2){
				...
				activityMonitor().verifyActivityMonitorForError(orderNumber, "NoError");
