...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchase.TC06_eMagazineSingleIssueAgencyHelper;
 ...
 * @author zsadeque
 */
public class TC06_eMagazineSingleIssueAgency extends TC06_eMagazineSingleIssueAgencyHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		ean = "2940000983874";
		logInfo("TC06_eMagazineSingleIssueAgency");
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersToExcel("TC06_eMagazineSingleIssueAgency", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC06_eMagazineSingleIssueAgency");
			customerID = getCustomerIDFromBNOrdersExcel("TC06_eMagazineSingleIssueAgency");
			...
			if(orderNumber.length()>2){
				...
				logError("Order Number is not found");
			}
