...
import resources.scripts.atlasProcessingOrders.physicalpurchase.TC01_NetworkDealerItemPurchaseHelper;
 ...
 * @author zsadeque
 */
public class TC01_NetworkDealerItemPurchase extends TC01_NetworkDealerItemPurchaseHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writePhysicalOrdersToExcel("TC01_NetworkDealerItemPurchase", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNPhysicalOrdersExcel("TC01_NetworkDealerItemPurchase");
			customerID = getCustomerIDFromBNPhysicalOrdersExcel("TC01_NetworkDealerItemPurchase");
