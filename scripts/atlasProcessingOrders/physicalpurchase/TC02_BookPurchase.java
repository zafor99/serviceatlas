...
import resources.scripts.atlasProcessingOrders.physicalpurchase.TC02_BookPurchaseHelper;
 ...
 * @author zsadeque
 */
public class TC02_BookPurchase extends TC02_BookPurchaseHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		ean = "9780439796644"; // remove temoporary
		//		ean = "9780300164992";
		if(AtlasScriptExecution.getScriptRun())
		{
			checkout().USPhysicalPurchase().submitOrder(true,"STD",ean);
			...
			if(orderNumber.length()>2){
				writePhysicalOrdersToExcel("TC02_BookPurchase", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNPhysicalOrdersExcel("TC02_BookPurchase");
			customerID = getCustomerIDFromBNPhysicalOrdersExcel("TC02_BookPurchase");
