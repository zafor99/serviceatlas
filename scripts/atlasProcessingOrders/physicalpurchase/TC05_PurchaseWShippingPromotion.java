...
import resources.scripts.atlasProcessingOrders.physicalpurchase.TC05_PurchaseWShippingPromotionHelper;
 ...
 * @author zsadeque
 */
public class TC05_PurchaseWShippingPromotion extends TC05_PurchaseWShippingPromotionHelper
{
	/**
	...
	{
		String xmlFile = "C:\\MyDevelopment\\Workspaces\\RFTWorkspace\\Atlas-QA-RFT\\testdata\\AtlasProcessingOrders\\physicalpurchase\\TC05_PurchaseWShippingPromotion.xml";
		String orderNumber,customerID,timeStamp,hr,min,ean = null;
		//ean = "9780300164992";
		ean = "9780439796644";
		if(AtlasScriptExecution.getScriptRun())
		{
			checkout().USPhysicalPurchase().submitOrder(true,"OPS",ean);
			customerID = checkout().USPhysicalPurchase().getCustomerId();
			orderNumber = checkout().USPhysicalPurchase().getOrderNumber();
			timeStamp = checkout().USPhysicalPurchase().getOrderTimeStamp();
			if(orderNumber.length()>2){
				writePhysicalOrdersToExcel("TC05_PurchaseWShippingPromotion", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNPhysicalOrdersExcel("TC05_PurchaseWShippingPromotion");
			customerID = getCustomerIDFromBNPhysicalOrdersExcel("TC05_PurchaseWShippingPromotion");
