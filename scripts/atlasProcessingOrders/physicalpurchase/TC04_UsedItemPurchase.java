...
import resources.scripts.atlasProcessingOrders.physicalpurchase.TC04_UsedItemPurchaseHelper;
 ...
 * @author zsadeque
 */
public class TC04_UsedItemPurchase extends TC04_UsedItemPurchaseHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writePhysicalOrdersToExcel("TC04_UsedItemPurchase", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNPhysicalOrdersExcel("TC04_UsedItemPurchase");
			customerID = getCustomerIDFromBNPhysicalOrdersExcel("TC04_UsedItemPurchase");
