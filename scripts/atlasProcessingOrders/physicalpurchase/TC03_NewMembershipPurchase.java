...
import resources.scripts.atlasProcessingOrders.physicalpurchase.TC03_NewMembershipPurchaseHelper;
 ...
 * @author zsadeque
 */
public class TC03_NewMembershipPurchase extends TC03_NewMembershipPurchaseHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		{
			checkout().USPhysicalPurchase(xmlFile).submitOrder(true,null,ean);
			...
			if(orderNumber.length()>2){
				writePhysicalOrdersToExcel("TC03_NewMembershipPurchase", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNPhysicalOrdersExcel("TC03_NewMembershipPurchase");
			customerID = getCustomerIDFromBNPhysicalOrdersExcel("TC03_NewMembershipPurchase");
