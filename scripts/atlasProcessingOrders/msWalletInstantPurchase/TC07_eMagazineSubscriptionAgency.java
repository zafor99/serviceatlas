...
import resources.scripts.atlasProcessingOrders.msWalletInstantPurchase.TC07_eMagazineSubscriptionAgencyHelper;
 ...
 * @author zsadeque
 */
public class TC07_eMagazineSubscriptionAgency extends TC07_eMagazineSubscriptionAgencyHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeMSWalletOrdersToExcel("TC07_eMagazineSubscriptionAgency", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromMSWalletOrdersExcel("TC07_eMagazineSubscriptionAgency");
			customerID = getCustomerIDFromMSWalletOrdersExcel("TC07_eMagazineSubscriptionAgency");
			if(orderNumber.length()>2){
