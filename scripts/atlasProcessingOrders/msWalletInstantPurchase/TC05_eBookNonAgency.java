...
import resources.scripts.atlasProcessingOrders.msWalletInstantPurchase.TC05_eBookNonAgencyHelper;
 ...
 * @author zsadeque
 */
public class TC05_eBookNonAgency extends TC05_eBookNonAgencyHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeMSWalletOrdersToExcel("TC05_eBookNonAgency", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromMSWalletOrdersExcel("TC05_eBookNonAgency");
			customerID = getCustomerIDFromMSWalletOrdersExcel("TC05_eBookNonAgency");
			if(orderNumber.length()>2){
