...
import resources.scripts.atlasProcessingOrders.msWalletInstantPurchase.TC06_ebookPreOrderHelper;
 ...
 * @author zsadeque
 */
public class TC06_ebookPreOrder extends TC06_ebookPreOrderHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeMSWalletOrdersToExcel("TC06_ebookPreOrder", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromMSWalletOrdersExcel("TC06_ebookPreOrder");
			customerID = getCustomerIDFromMSWalletOrdersExcel("TC06_ebookPreOrder");
			ean = getEANFromMSWalletOrdersExcel("TC06_ebookPreOrder");
			if(orderNumber.length()>2){
