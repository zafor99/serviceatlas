...
import resources.scripts.atlasProcessingOrders.msWalletInstantPurchase.TC03_eBookPubItHelper;
 ...
 * @author zsadeque
 */
public class TC03_eBookPubIt extends TC03_eBookPubItHelper
{
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeMSWalletOrdersToExcel("TC03_eBookPubIt", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromMSWalletOrdersExcel("TC03_eBookPubIt");
			customerID = getCustomerIDFromMSWalletOrdersExcel("TC03_eBookPubIt");
			if(orderNumber.length()>2){
