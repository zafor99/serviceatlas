...
import resources.scripts.atlasProcessingOrders.msWalletInstantPurchase.TC10_eSubscriptionNonAgencyHelper;
 ...
 * @author zsadeque
 */
public class TC10_eSubscriptionNonAgency extends TC10_eSubscriptionNonAgencyHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeMSWalletOrdersToExcel("TC10_eSubscriptionNonAgency", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromMSWalletOrdersExcel("TC10_eSubscriptionNonAgency");
			customerID = getCustomerIDFromMSWalletOrdersExcel("TC10_eSubscriptionNonAgency");
			if(orderNumber.length()>2){
