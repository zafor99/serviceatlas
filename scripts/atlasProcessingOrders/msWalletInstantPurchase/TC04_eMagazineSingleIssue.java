...
import resources.scripts.atlasProcessingOrders.msWalletInstantPurchase.TC04_eMagazineSingleIssueHelper;
 ...
 * @author zsadeque
 */
public class TC04_eMagazineSingleIssue extends TC04_eMagazineSingleIssueHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeMSWalletOrdersToExcel("TC04_eMagazineSingleIssue", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromMSWalletOrdersExcel("TC04_eMagazineSingleIssue");
			customerID = getCustomerIDFromMSWalletOrdersExcel("TC04_eMagazineSingleIssue");
			if(orderNumber.length()>2){
