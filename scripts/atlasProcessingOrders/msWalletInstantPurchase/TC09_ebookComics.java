...
import resources.scripts.atlasProcessingOrders.msWalletInstantPurchase.TC09_ebookComicsHelper;
 ...
 * @author zsadeque
 */
public class TC09_ebookComics extends TC09_ebookComicsHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeMSWalletOrdersToExcel("TC09_ebookComics", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromMSWalletOrdersExcel("TC09_ebookComics");
			customerID = getCustomerIDFromMSWalletOrdersExcel("TC09_ebookComics");
			if(orderNumber.length()>2){
