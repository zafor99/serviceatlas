...
import resources.scripts.atlasProcessingOrders.msWalletInstantPurchase.TC08_ebookKidsHelper;
 ...
 * @author zsadeque
 */
public class TC08_ebookKids extends TC08_ebookKidsHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeMSWalletOrdersToExcel("TC08_ebookKids", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromMSWalletOrdersExcel("TC08_ebookKids");
			customerID = getCustomerIDFromMSWalletOrdersExcel("TC08_ebookKids");
			if(orderNumber.length()>2){
