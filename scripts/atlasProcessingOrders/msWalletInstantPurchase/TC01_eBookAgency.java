...
import resources.scripts.atlasProcessingOrders.msWalletInstantPurchase.TC01_eBookAgencyHelper;
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeMSWalletOrdersToExcel("TC01_eBookAgency", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromMSWalletOrdersExcel("TC01_eBookAgency");
			customerID = getCustomerIDFromMSWalletOrdersExcel("TC01_eBookAgency");
			if(orderNumber.length()>2){
