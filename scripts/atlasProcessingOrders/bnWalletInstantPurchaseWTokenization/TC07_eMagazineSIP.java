...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchaseWTokenization.TC07_eMagazineSIPHelper;
 ...
 * @author zsadeque
 */
public class TC07_eMagazineSIP extends TC07_eMagazineSIPHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersWTokenizationToExcel("TC07_eMagazineSIP", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersWTokenizationExcel("TC07_eMagazineSIP");
			customerID = getOrderNumberFromBNOrdersWTokenizationExcel("TC07_eMagazineSIP");
