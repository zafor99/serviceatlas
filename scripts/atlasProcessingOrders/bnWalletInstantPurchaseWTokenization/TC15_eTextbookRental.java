...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchaseWTokenization.TC15_eTextbookRentalHelper;
 ...
 * @author zsadeque
 */
public class TC15_eTextbookRental extends TC15_eTextbookRentalHelper
{
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		ean = "2940000907566";//9780495896807
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersWTokenizationToExcel("TC15_eTextbookRental", timeStamp, orderNumber, ean,customerID);
			}
			else{
				writeBNOrdersWTokenizationToExcel("TC15_eTextbookRental", timeStamp, "1", ean,customerID);
				logError("Order Number is not generated");
			}	
		}

		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersWTokenizationExcel("TC15_eTextbookRental");
			customerID = getOrderNumberFromBNOrdersWTokenizationExcel("TC15_eTextbookRental");
