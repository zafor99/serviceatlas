...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchaseWTokenization.TC03_ebookPreOrderHelper;
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersWTokenizationToExcel("TC03_ebookPreOrder", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersWTokenizationExcel("TC03_ebookPreOrder");
			customerID = getOrderNumberFromBNOrdersWTokenizationExcel("TC03_ebookPreOrder");
