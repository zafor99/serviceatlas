...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchaseWTokenization.TC16_freeEBookHelper;
 ...
 * @author zsadeque
 */
public class TC16_freeEBook extends TC16_freeEBookHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersWTokenizationToExcel("TC16_freeEBook", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersWTokenizationExcel("TC16_freeEBook");
			customerID = getOrderNumberFromBNOrdersWTokenizationExcel("TC16_freeEBook");
