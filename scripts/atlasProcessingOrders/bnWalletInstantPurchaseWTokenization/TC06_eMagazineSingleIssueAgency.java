...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchaseWTokenization.TC06_eMagazineSingleIssueAgencyHelper;
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersWTokenizationToExcel("TC06_eMagazineSingleIssueAgency", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersWTokenizationExcel("TC06_eMagazineSingleIssueAgency");
			customerID = getOrderNumberFromBNOrdersWTokenizationExcel("TC06_eMagazineSingleIssueAgency");
