...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchaseWTokenization.TC10_eNewspaperSubLibreAggregatorHelper;
 ...
 * @author zsadeque
 */
public class TC10_eNewspaperSubLibreAggregator extends TC10_eNewspaperSubLibreAggregatorHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		ean = "2940000281581";//"2940000446416";
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersWTokenizationToExcel("TC10_eNewspaperSubLibreAggregator", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersWTokenizationExcel("TC10_eNewspaperSubLibreAggregator");
			customerID = getOrderNumberFromBNOrdersWTokenizationExcel("TC10_eNewspaperSubLibreAggregator");
