...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchaseWTokenization.TC11_eNewspaperSubscriptionAgencyHelper;
 ...
 * @author zsadeque
 */
public class TC11_eNewspaperSubscriptionAgency extends TC11_eNewspaperSubscriptionAgencyHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersWTokenizationToExcel("TC11_eNewspaperSubscriptionAgency", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersWTokenizationExcel("TC11_eNewspaperSubscriptionAgency");
			customerID = getOrderNumberFromBNOrdersWTokenizationExcel("TC11_eNewspaperSubscriptionAgency");
