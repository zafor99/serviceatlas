...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchaseWTokenization.TC19_NookVideoHelper;
 ...
 * @author zsadeque
 */
public class TC19_NookVideo extends TC19_NookVideoHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException, IOException 
	{
		...
		{
			instantPurchase().submitIPVideoPurchase(ean);
			...
			if(orderNumber.length()>2){
				writeBNOrdersWTokenizationToExcel("TC19_NookVideo", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersWTokenizationExcel("TC19_NookVideo");
			customerID = getOrderNumberFromBNOrdersWTokenizationExcel("TC19_NookVideo");
