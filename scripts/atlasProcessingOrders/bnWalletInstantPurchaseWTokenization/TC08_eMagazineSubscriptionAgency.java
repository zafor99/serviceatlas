...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchaseWTokenization.TC08_eMagazineSubscriptionAgencyHelper;
 ...
 * @author zsadeque
 */
public class TC08_eMagazineSubscriptionAgency extends TC08_eMagazineSubscriptionAgencyHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersWTokenizationToExcel("TC08_eMagazineSubscriptionAgency", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersExcel("TC08_eMagazineSubscriptionAgency");
			customerID = getCustomerIDFromBNOrdersExcel("TC08_eMagazineSubscriptionAgency");
