...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchaseWTokenization.TC01_eBookAgencyHelper;
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersWTokenizationToExcel("TC01_eBookAgency", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			...
			if(orderNumber.length()>2){
				...
				//Validate no Error in Activity Monitor for this order
