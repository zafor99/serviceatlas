...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchaseWTokenization.TC18_NookAppHelper;
 ...
 * @author zsadeque
 */
public class TC18_NookApp extends TC18_NookAppHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersWTokenizationToExcel("TC18_NookApp", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersWTokenizationExcel("TC18_NookApp");
			customerID = getOrderNumberFromBNOrdersWTokenizationExcel("TC18_NookApp");
