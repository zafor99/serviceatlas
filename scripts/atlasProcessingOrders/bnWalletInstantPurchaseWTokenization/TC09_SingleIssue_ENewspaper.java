...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchaseWTokenization.TC09_SingleIssue_ENewspaperHelper;
 ...
 * @author zsadeque
 */
public class TC09_SingleIssue_ENewspaper extends TC09_SingleIssue_ENewspaperHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersWTokenizationToExcel("TC09_SingleIssue_ENewspaper", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersWTokenizationExcel("TC09_SingleIssue_ENewspaper");
			customerID = getOrderNumberFromBNOrdersWTokenizationExcel("TC09_SingleIssue_ENewspaper");
