...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchaseWTokenization.TC17_eBookNonAgencyHelper;
 ...
 * @author zsadeque
 */
public class TC17_eBookNonAgency extends TC17_eBookNonAgencyHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersWTokenizationToExcel("TC17_eBookNonAgency", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersWTokenizationExcel("TC01_eBookAgency");
			customerID = getOrderNumberFromBNOrdersWTokenizationExcel("TC01_eBookAgency");
