...
import resources.scripts.atlasProcessingOrders.bnWalletInstantPurchaseWTokenization.TC04_eBookPubItHelper;
 ...
 * @author zsadeque
 */
public class TC04_eBookPubIt extends TC04_eBookPubItHelper
{
	/**
	...
	public void testMain(Object[] args) throws ClassNotFoundException 
	{
		...
		if(AtlasScriptExecution.getScriptRun())
		{
			...
			if(orderNumber.length()>2){
				writeBNOrdersWTokenizationToExcel("TC04_eBookPubIt", timeStamp, orderNumber, ean,customerID);
			}
			else{
		...
		if (AtlasScriptExecution.getDBValidation()){
			orderNumber = getOrderNumberFromBNOrdersWTokenizationExcel("TC04_eBookPubIt");
			customerID = getOrderNumberFromBNOrdersWTokenizationExcel("TC04_eBookPubIt");
			if(orderNumber.length()>2){
